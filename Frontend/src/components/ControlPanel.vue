  ...
  <div class="control-panel">
    ...
    <div v-if="objectCount > 0" class="dynamic-controls">
      ...
      <div class="object-buttons">
        <button v-for="i in objectCount"
                :key="i"
                @click="waterSpecific(i)"
                :disabled="isLoading"
...
.control-panel {
    ...
    max-width: 1000px; /* ⬅️ NEW */
    width: 90%; 
    margin: 30px auto; /* ⬅️ NEW */
    padding: 40px; /* ⬅️ NEW */
...
h2 {
    font-size: 2rem; /* ⬅️ NEW */
    text-align: center;
    color: #333;
    margin-bottom: 30px; /* ⬅️ NEW */
}

h3 {
    font-size: 1.5rem; /* ⬅️ NEW */
    color: #42b983;
    margin-top: 30px; /* ⬅️ NEW */
}

/* Flex Container สำหรับปุ่ม - คงเดิม (Flexbox ดีอยู่แล้ว) */
.main-controls, .object-buttons {
    display: flex;
    flex-wrap: wrap;
    gap: 15px; /* ⬅️ เพิ่ม Gap ระหว่างปุ่มเล็กน้อย */
    justify-content: center;
}

/* Button Styles */
button {
    padding: 15px 30px; /* ⬅️ NEW */
    border: none;
    border-radius: 6px;
    cursor: pointer;
    font-weight: 600;
    transition: background-color 0.3s, transform 0.1s;
    min-width: 180px; /* ⬅️ NEW */
    flex-grow: 1; 
}
...
.btn-detect { background-color: #007bff; color: white; }
.btn-water-all { background-color: #42b983; color: white; }
.btn-specific { background-color: #64b5f6; color: white; }
.btn-specific.highlight { background-color: #ff9800; border: 3px solid #e65100; color: #333; }
.image-area { text-align: center; margin: 25px 0; }
.result-image { max-width: 100%; height: auto; border: 2px solid #ccc; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); }
.status-message { text-align: center; padding: 10px; border-radius: 4px; font-style: italic; margin-top: 15px; }
.status-message.loading { background-color: #fff3cd; color: #856404; }
.status-message.info { background-color: #e2e3e5; color: #666; }
